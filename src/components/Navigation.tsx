...
const Navigation = ({ blok }: { blok: NavigationStoryblok | null }) => {
  const renderDrowpdownButton = (item: MenuItemStoryblok) => (
    <DropdownButton
      ...
      key={item._uid}
    >
      {item?.submenu_item?.map((item) => (
        <Dropdown.Item href={`/${item?.link?.cached_url}`} key={item._uid}>
          {item.title}
        </Dropdown.Item>
      ))}
    </DropdownButton>
  ...
  const renderNavigationDropDown = (item: MenuItemStoryblok) => {
    return (
      <NavDropdown title={item.title} id="basic-nav-dropdown" key={item._uid}>
  ...
  const renderNavigationItems = () => {
    const items = blok?.menu_item?.map((item) => {
      ...
      return (
        <Nav.Link href={`/${item.link?.cached_url}`} key={item._uid}>
          {item.title}
        </Nav.Link>
      );
    });
    ...
    <Navbar expand="lg" className="bg-body-tertiary">
      <Container>
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="me-auto">{renderNavigationItems()}</Nav>
        </Navbar.Collapse>
      </Container>
    </Navbar>
  );
